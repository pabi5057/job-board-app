    ...
    return (
        <div className="mt-20 mb-12">
            <div className="block sm:flex items-center justify-between w-[80%] mx-auto">
                ...
                {session?.user?.role=='candidate' && <ApplyButton job={singleJob}/>}
                {!session && (
            ...
            <div className="mt-16 w-[80%] mx-auto">
                ...
                <p className="mt-4 text-black/70 ">{singleJob?.description}</p>             
                <h1 className="text-[20px] font-semibold mt-8">Skills</h1>
                  <ul className="mt-4">
                    {
                        singleJob?.skills?.map((skill,index)=>(
                          <li key={index} className="mt-4 text-black/70" >{skill}</li>
                        ))
                    }                   
